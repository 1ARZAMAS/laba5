...
void createPlane(vector<string>& towns, vector<string>& planes, map<string, vector<string>>& route) {
    string plane, stop; // самолет и остановки
    ...
    if (find(planes.begin(), planes.end(), plane) != planes.end()) {
        cout << "Самолет с таким названием уже существует. Пожалуйста, введите другое название\n" << endl;
        return;
    }
    ...
    getline(cin, stop);  // закинули строку
    string town;
    int town_count = 0;
    vector<string> new_towns; // вектор для новых городов

    for (auto a : stop) { // добавляем все города
        if (a != ' ') {
            town.push_back(a);
    ...
    if (adjacent_find(new_towns.begin(), new_towns.end()) != new_towns.end()) {
        cout << "Введены одинаковые города. Пожалуйста, введите разные города\n" << endl;
    ...
    for (const auto& pair : route) {
        if (pair.second == new_towns && pair.first != plane) {
			planes.pop_back(); //удаляем этот самолет
            cout << "Самолет с таким же маршрутом уже существует\n" << endl;
            return;
        }
    }
    ...
    if (town_count == 0) {
        cout << "Введен только один город. Пожалуйста, введите два или более городов.\n" << endl;
        for (const auto& new_town : new_towns) {
			planes.pop_back(); //удаляем этот самолет
